"""
Random Forest training script that mirrors the RAM-friendly pipeline used by the CNN:
- Reuses the pre-split metadata from artifacts generated by load_files.py
- Streams images in batches to build feature matrices without loading everything at once
- Scales features and trains/evaluates a RandomForestClassifier
"""
...
IMAGE_SIZE = (256, 256)
FEATURE_SIDE = 64
FEATURE_SIZE = FEATURE_SIDE * FEATURE_SIDE
DEFAULT_BATCH = 256


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a RandomForest on PlantVillage with streaming feature extraction.")
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH, help="Number of images to load per batch.")
...
def _extract_batch_features(paths: Iterable[str]) -> np.ndarray:
    batch = np.empty((len(paths), FEATURE_SIZE), dtype=np.float32)
    for i, path in enumerate(paths):
        ...
        small = cv2.resize(gray, (FEATURE_SIDE, FEATURE_SIDE))
        batch[i] = small.flatten()
    return batch
...
def build_feature_store(df: pd.DataFrame, label_map: Dict[str, int], split: str, batch_size: int) -> Tuple[Path, np.ndarray]:
    ...
    for start, end in iter_chunks(count, batch_size):
        rows = df.iloc[start:end]
        feats = _extract_batch_features(rows["path"].tolist())
        features[start:end] = feats
        labels[start:end] = rows["label"].map(label_map).to_numpy(dtype=np.int32)
